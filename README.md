...
- **Продвинутая функция [log](#log---одна-функция-чтобы-править-всеми)**, которую можно использовать "[как есть](#ручное-логирование)", а можно как [контекстный менеджер](#контекстный-менеджер) или даже как декоратор для [функций](#декорируем-функции) и (в том числе корутинных) [классов](#декорируем-классы).
...
- Есть **более 570 тестов**, которые обеспечивают покрытие более чем на **96%**. И стремление эти цифры еще увеличить.
...
В качестве [контекстного менеджера](#контекстный-менеджер) или декоратора [для функций](#декорируем-функции) (в том числе корутинных) и [классов](#декорируем-классы):
...
Функция сама понимает контекст, в котором она была вызвана! И подстраивает свое поведение соответственно. Во всех примерах выше сообщение лога передается первым неименованным аргументом. Если вам нечего сказать, можно использовать ```log``` и без скобок:
...
Колонки **exception_message** и **exception_type** тут тоже заполнятся автоматически. Флаг ```success``` будет установлен в значение False. Трейсбек и локальные переменные той функции, где произошла ошибка, заполнятся автоматически.
