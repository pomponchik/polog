...
Еще одна важная вещь, которую вам нужно знать про ```@flog```: этот декоратор нельзя применять по отношению к динамически создаваемым в процессе работы программы функциям. К примеру, к лямбдам. Причина в том, что "под капотом" в Polog происходит регистрация каждой функции в специальном реестре по ее id. Если ваша функция существует в программе как временный объект, рано или поздно выделенная под нее интерпретатором Python память будет освобождена, а ее id перестанет указывать на прежний объект. И тут возникнут 2 проблемы. Во-первых, это означает потенциальную утечку памяти, поскольку из реестра id функции не удаляется. Во-вторых, бывает так, что, когда память из-под одной функции освобождается, ровно туда же записывается новая функция. С точки зрения реестра Polog, новая функция ему уже знакома, в то время как в действительности это совершенно новый объект функции. Это может приводить к непредсказуемому поведению Polog по отношению к новой функции. В общем, не берите это все в голову, главное не делайте вот так:
...
Применяя наследование классов, ```@clog``` стоит навешивать только на крайние классы в иерархии, то есть на те, у которых нет наследников. Это связанно с особенностями механизма наследования в Python. При обращении к атрибуту класса по имени, интерпретатор ищет его сначала в текущем классе, затем выше по иерархии MRO, потом еще выше, и так далее. Это может приводить к трудно отлаживаемым ошибкам. Вы наследуете класс __вместе с навешенным на него логированием__, и это логирование не знает, что работает уже не в оригинальном классе, а в наследнике. Если на наследника вы тоже навесите ```@clog```, то определенные в нем методы будут подчиняться правилам логирования, указанным в его декораторе, а те методы, что отнаследованы - будут покрыты логированием из декоратора класса-родителя. В результате у вас в половине класса логирование работает по одним правилам, а в другой половине - по другим.
