...
- Можно декорировать [функции](#декорируем-функции) (в том числе корутинные) и [классы](#декорируем-классы), оборачивать код в [контекстный менеджер](#контекстный-менеджер), и даже вызывать логгер вручную. И все это через **одну и ту же [функцию](#log---одна-функция-чтобы-править-всеми)**.
...
- Можно **редактировать лог, записываемый декоратором, изнутри той функции, которую он оборачивает**. [Эта возможность](#редактируем-автоматические-логи-из-задекорированных-функций) делает логирование через декоратор таким же мощным, как обычное, и в то же время - гораздо более минималистичным.
...
Чего в нем примечательного? В данном случае ошибка происходит в функции division(), а затем, поднимаясь по стеку вызовов, она проходит через функцию operation(). Однако логгер записал сообщение об ошибке только один раз! Встретив исключение в первый раз, он его записывает и ставит на нем специальную метку, чтобы игнорировать в дальнейшем.
...
Немного более сложный случай. Мы навешиваем ```@unlog``` на функцию, из которой вызывается вторая функция, и вот на той второй функции логирование тоже перестанет работать:
