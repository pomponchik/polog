...
Разработанный полностью с нуля, он переосмысливает и упрощает многие моменты, связанные с логированием. С ним вы можете выбросить ненужный код из своих проектов. Судите сами:

- **Легко использовать**. Главный конкурент Polog по простоте - функция [print()](https://docs.python.org/3/library/functions.html#print) из стандартной библиотеки, при этом по возможностям даже [logging](https://docs.python.org/3/library/logging.html) далеко позади.
- [**Просто настраивать**](#общие-настройки).
- Можно декорировать [функции](#декорируем-функции) (в том числе корутинные) и [классы](#декорируем-классы), оборачивать код в [контекстный менеджер](#контекстный-менеджер), и даже вызывать логгер [вручную](#ручное-логирование). И все это через **одну и ту же [функцию](#log---одна-функция-чтобы-править-всеми)**.
...
- Ядро Polog [поддерживает **многопоточность**](#движки-синхронный-и-асинхронный), иногда это может заметно ускорить программу. В этом режиме запись лога становится **неблокирующей операцией**.
- **Система уровней очень гибкая**. Можно создавать собственные [уровни логирования](#уровни-логирования), давать им произвольные имена и пользоваться ими так, будто они были встроены изначально. В большинстве других логгеров уровни "прибиты гвоздями".
- **Легко модифицировать**. Можно писать [собственные обработчики](#пишем-свой-обработчик), а можно добавлять собственную логику для [автоматического заполнения](#добавляем-извлекаемые-поля) полей лога.
- У логгера **нет внешних зависимостей**.
- Есть **более 550 тестов**, которые обеспечивают покрытие более чем на 96%. И стремление эти цифры еще увеличить.
...
1. Прежде всего, поток событий из программы нужно как-то ловить. Для этого существует специальная функция - [log](#log---одна-функция-чтобы-править-всеми). Вы можете ее использовать в качестве [декоратора](#декорируем-функции) (в том числе [для классов](#декорируем-классы)) или [контекстного менеджера](#контекстный-менеджер), а можете напрямую писать [сообщения в нее](#ручное-логирование). Фактически вам нужен всего один импорт в любом модуле вашей программы, чтобы логировать любые события любым из доступных способов.
...
Описанная выше схема является типичной для логгеров. Отличие конкретно Polog от остальных заключается прежде всего в лаконичности синтаксиса и универсальности [одной функции](#log---одна-функция-чтобы-править-всеми), то есть по сути в проработанности первого пункта схемы.
...
- **```set()```**: общие настройки логгера.
  ...
  Метод ```set()``` принимает следующие именованные параметры:
    ...
    **integration_with_logging** (bool) - интеграция с модулем [logging](https://docs.python.org/3/library/logging.html) из стандартной библиотеки. При значении ```True``` (по умолчанию) записи логов из ```logging``` копируются в Polog.
...
Одно из главных преимуществ Polog - фреймворк предоставляет несколько способов собирать логи через одну и ту же функцию. Импортируется она так:

```python
from polog import log
```

Для удобства через [настройки](#общие-настройки) вы можете включить режим, при котором эта функция будет доступна по всей программе без дополнительных импортов:
...
В качестве декоратора [для функций](#декорируем-функции) (в том числе корутинных):
...
А также в качестве [контекстного менеджера](#контекстный-менеджер):
...
```python
log('plain text', level=5) # Работает при любом способе регистрации логов, в том числе через декораторы и контекстные менеджеры.
```

Также можно при любом способе использования ```log``` указывать уровень логирования через точку (предварительно [зарегистрировав](#уровни-логирования) это имя):

```python
config.levels(some_level=5)

log.some_level('plain text') # Это тоже работает в том числе и для декораторов, контекстных менеджеров.
```
...
К каждому логу можно добавить произвольный текст, передав его первым неименованным аргументом:
...
Еще один удобный способ записывать логи - оборачивать блоки кода в контекстные менеджеры. В Polog эта возможность встроена прямо в универсальную функцию [```log()```](#log---одна-функция-чтобы-править-всеми) - достаточно лишь использовать ее соответствующим образом:
...
Контекстный менеджер засекает время работы обернутого им блока кода. Кроме того, он автоматически записывает информацию о случившихся внутри него исключениях.

В реальных программах очень часто встречаются случаи, когда логгер вызывается внутри блока try-except (или хуже того, ошибки просто [подавляются](https://en.wikipedia.org/wiki/Error_hiding) без всяких следов, даже в виде логов). С использованием стандартного logging это обычно выглядит примерно так:
