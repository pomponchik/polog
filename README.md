...
Вы можете также использовать ее в любом месте программы, не импортируя дополнительно. Для этого вызовите один раз метод ```set_log_as_built_in()```:
...
В нескольких разделах ниже вы можете узнать подробности о том, как она работает.
