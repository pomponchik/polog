...
1. Прежде всего, поток событий из программы нужно как-то ловить. Для этого существует специальная функция - [log](#log---одна-функция-чтобы-править-всеми). Вы можете ее исьзовать в качестве [декоратора](#декорируем-функции) (в том числе [для классов](#декорируем-классы)), а можете напрямую писать [сообщения в нее](#ручное-логирование). Фактически вам нужен всего один импорт в любом модуле вашей программы, чтобы логировать любые события любым из доступных способов.
...
1. Логирование заставляет писать сильно больше кода, причем этот код весьма шаблонен и несколько базовых паттернов (таких, как отдельные сообщения на начало и конец каждой операции, или обрамление логами блоков try-except) переисьзуются повсюду. То есть логи отнимают самое главное - время программиста.
...
В данном примере мы к логу, который в любом случае был бы сформирован по факту вынения функции, добавили сообщение.
...
Это может быть вам езно, поскольку часто ошибки важнее прочих событий в программе, и вы можете сделать так, чтобы только они проходили через "фильтр" общего уровня логирования.
...
Уровням логирования можно присвоить имена и в дальнейшем исьзовать их вместо чисел:
...
Если вы привыкли ьзоваться стандартным модулем [logging](https://docs.python.org/3.8/library/logging.html), вы можете присвоить уровням логирования [стандартные имена](https://docs.python.org/3.8/library/logging.html#logging-levels) оттуда:
...
- **```set()```**: общие настройки логгера.
  ...
  Метод ```set()``` принимает следующие именованные параметры:
    ...
    **unknown_fields_in_handle_logs** (bool) - можно ли передавать при ручном логировании функцией [```log()```](#ручное-логирование) аргументы с неизвестными именами. По умолчанию можно.
...
- **```set_log_as_built_in()```**: устанавливаем функцию [```log()```](#log---одна-функция-чтобы-править-всеми) как одну из [встроенных](https://docs.python.org/3/library/functions.html). После вызова данного метода, ```log()``` можно использовать в любом месте программы без импорта.
...
Вы можете также использовать его в любом месте программы, не импортируя дополнительно. Для этого вызовите один раз метод ```set_log_as_built_in()```:
...
Если в задекорированной функции возникло необработанное исключение, в первый раз ```@log``` записывает его, но когда оно поднимется выше по стеку вызовов функций и встретится с ```@log``` снова - будет проигнорировано. Изменить это можно через [настройку](#общие-настройки) ```deduplicate_errors```.
