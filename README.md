...
Мы также можем отключить блокировки полностью, передав в качестве данного аргумента ```None```:

```python
handler = file_writer('file.log', rotation='200 megabytes >> archive', lock_type=None)
```

Или включить только один вид блокировки:
...
По умолчанию в Polog включен только один вид блокировки - блокировка потока. Это связано с тем, что так не требуется учитывать нюансы доступности системных вызовов под разными ОС, и кроме того, обычный мьютекс просто быстрее.
