...
Расширением будет считаться любая функция, которая принимает в себя набор определенных именованных аргуметов. Набор переменных, которые она должна принимать, аналогичен набору полей в базе данных, который вы можете увидеть в разделе [Общая информация о логгере](#подробности) (кроме ```id```). Эта функция будет вызываться при каждом событии, которое должно быть залогировано в БД.
...
В составе Polog есть ряд уже готовых расширений. Одно из них позволяет вам настроить отправку электронных писем по SMTP-протоколу. Вам это может пригодиться для быстрого реагирования на какие-то особо критичные события в вашем коде.
