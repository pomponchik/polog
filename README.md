...
Контекстный менеджер засекает время работы обернутого им блока кода. Кроме того, он автоматически записывает информацию случившихся внутри него исключениях.

Очень часто в программах встречаются случаи, когда логгер вызывается внутри блока try-except (или хуже того, когда ошибка просто [подавляется](https://en.wikipedia.org/wiki/Error_hiding) без всяких следов, даже в виде логов).
