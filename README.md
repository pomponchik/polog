...
- **Поддержка асинхронных функций**. Декораторы для автоматического логирования работают как на обычных функциях, так и на корутинах.
...
- Вы можете писать собственные **обработчики** или пользоваться уже существующими. К примеру, вы можете настроить [отправку](#включаем-оповещения-по-электронной-почте) уведомлений об ошибках по электронной почте или их запись в реляционную БД.
...
- **```set()```**: общие настройки логгера.
  ...
  Принимает следующие именованные параметры:
    ...
    **level** (int, str) - общий уровень логирования. События уровнем ниже записываться не будут. Подробнее в разделе "[уровни логирования](#уровни-логирования)".

    **errors_level** (int, str) - уровень логирования для ошибок. По умолчанию он равен 2-м. Также см. в "[уровни логирования](#уровни-логирования)".
...
Обработчик - это любая функция, которая принимает в себя набор определенный набор аргументов. Этот набор аналогичен набору полей в базе данных, который вы можете увидеть в разделе [общей информации о логгере](#подробности) (кроме ```id```). Эта функция будет вызываться при каждом событии, которое должно быть залогировано в БД.
...
Кроме того, что вы написали расширение, его еще нужно зарегистрировать в движке Polog. Для этого достаточно передать вашу функцию в метод "add_handlers":
...
# Передаем ваше расширение в Polog. В метод add_handlers() можно передать несколько функций подряд.
