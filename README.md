...
Заполнение поля происходит в 2 этапа:
...
Теперь при каждом логировании в обработчик логов будет передаваться среди прочих аргумент под названием "ip", значением которого будет извлеченный из строки запроса ip-адрес.
...
Вызов заполнителей дополнительных полей экранируется от ошибок самим Polog. В случае неудачи извлечения - например, если вы пытаетесь извлечь из функции аргумент, которого в ней нет - ничего просто не произойдет, все остальное запишется как положено. Будьте осторожны при использовании изменяемых типов данных из аргументов функции при извлечении дополнительных полей. Вы рискуете зааффектить логгером поведение оригинальных функций, если будете как-то изменять в процессе исходные данные. Кроме того, изменяемые типы данных на момент вашего к ним обращения могут быть уже изменены относительно того состояния, в каком они были при вызове первоначальной функции.
