...
- **```set()```**: общие настройки логгера.
  ...
  Метод ```set()``` принимает следующие именованные параметры:
    ...
    **json_module** (module) - модуль для обработки данных в формате [json](https://en.wikipedia.org/wiki/JSON). Обязательно должен включать 2 функции: ```loads()``` и ```dumps()```. Для ускорения работы рекомендуется передать сюда модуль из библиотеки [ujson](https://pypi.org/project/ujson/1.35/).
...
Движок Polog - это некий объект, через который проходят все логи. Его задача - взять лог и передать его в обработчики.
