...
- Вы можете писать собственные **обработчики** или пользоваться уже существующими. К примеру, вы можете настроить [отправку](#включаем-оповещения-по-электронной-почте) уведомлений об ошибках по электронной почте.
...
Еще один встроенных [обработчиков](#обработчики) Polog позволяет вам настроить отправку электронных писем по [SMTP](https://en.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol)-протоколу. Вам это может пригодиться для быстрого реагирования на какие-то особо критичные события в вашем коде.
...
В самом простом случае, наследуясь от данного класса, вам достаточно переопределить всего 2 метода, чтобы получить полностью рабочий обработчик. Вот названия и сигнатуры этих методов:
...
Вот пример настолько суперпростого обработчика, который, однако, будет работать:
...
Все будет немного сложнее, если инициализация вашего обработчика требует каких-то дополнительных аргументов. Для этого вам придется переопределить метод ```__init__()``` базового класса:
...
В данном примере метод ```__init__()``` базового класса сделает со знакомыми ему аргументами все, что необходимо. Например, для функций - валидирует их на соответствие сигнатуры. Если вы хотите и для своих аргументов ввести какую-то валидацию, для этого в базовом классе также есть шорткат:

```python
class DefendedInputHandler():
...
Как видно на примере, для добавления валидации произвольных аргументов, необходимо сделать 2 вещи: 1. разместить в теле класса словарь под названием ```input_proves```, в котором ключи - это названия аргументов, а значения - функции, которые принимают эти аргументы и возвращают bool'еан, означающий, что данный аргумент прошел / не прошел проверку; 2. вызвать метод ```do_input_proves()```, передав ему в качестве именованных аргументов все переменные, которые необходимо провалидировать.
...
Если вы считаете, что он может быть полезен кому-то еще, опубликуйте его на [pypi.org](https://pypi.org/)! При этом не забудьте приложить к нему инструкцию, как им пользоваться. При наименовании пакетов рекомендуем соблюдать единый формат: ```{micro-description}_polog_handler```, например ```console_polog_handler```. Часть перед "_polog_handler" должна описывать механизм его работы или место назначения, куда отправляются логи, и ей не стоит быть больше 1-3 слов. Публикуя свой проект на github, вы также можете указать ему тег [```polog```](https://github.com/topics/polog), чтобы его можно было увидеть в соответствующем топике.
