...
- **```get_handlers()```**: получить [дерево обработчиков](#пространства-имен-и-иерархия-обработчиков).
...
Определение функции-обработчика должно выглядеть примерно так:
...
Каждый обработчик принимает 1 аргумент: объект лога. Лог по предоставляемому интерфейсу очень похож на словарь. Любое поле лога можно извлечь, пользуясь синтаксисом квадратных скобок - как из словаря. Также поддерживаются некоторые базовые операции, доступные со словарями. Примеры:
