...
Разработанный полностью с нуля, он переосмысливает и упрощает многие моменты, связанные с логированием. С ним вы можете выбросить ненужный код из своих проектов. Вот некоторые из фич:
...
- **Автоматическое логирование через универсальный декоратор**, который работает как с [функциями и методами](#декорируем-функции), так и с [целыми классами](#декорируем-классы). Это снижает трудозатраты для программиста, и в то же время делает логи более стандартизированными и качественными.
...
- **Высокая производительность**. Ядро Polog - многопоточное, что в некоторых ситуациях может заметно повысить производительность программы. Каждая запись лога является неблокирующей операцией для основного потока. TODO: пофиксить про многопоточное ядро.
...
- **```set()```**: общие настройки логгера.
  ...
  TODO: описать политику смены настроек (возможность менять в рантайме, транзакционные настройки)

  Принимает следующие именованные параметры:

  TODO: дописать новые пункты настроек
...
TODO: теперь не нужно устанавливать до первого вызова.
...
TODO: описать смену движков, принципы их работы
