...
- [**Простая настройка**](#общие-настройки).
- **Упор на декораторы**, которые здесь гораздо функциональнее, чем у большинства подобных библиотек.
- **Универсальный логгер** - одна и та же универсальная функция может использоваться как для [ручного логирования](#ручное-логирование), так и для декорирования [функций](#декорируем-функции) или даже [классов](#декорируем-классы), и даже работает как [контекстный менеджер](#контекстный-менеджер).
- **[Защита от дублирования](#дедупликация-исключений) записей об ошибках** при проходе исключения через стек вызовов, если вы используете декоратор логирования.
...
- **Гарантия безопасности данных**. Некоторые данные не должны попадать в логи из соображений безопасности. Если для вас это актуально - вы можете [защитить от этого отдельные функции](#запрет-логирования-через-декоратор-unlog).
