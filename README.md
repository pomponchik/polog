...
- Более 520 тестов, которые обеспечивают покрытие более чем на 95%. И есть стремление эти цифры еще увеличить.
...
Прежде, чем вызывать логгер, необходимо зарегистрировать [обработчик](#обработчики):
...
Чего в нем примечательного? В данном случае ошибка происходит в функции ```division()```, а затем, поднимаясь по стеку вызовов, она проходит через ```operation()```. Однако логгер записал сообщение об ошибке только один раз! Встретив исключение в первый раз, он его записывает и ставит на нем специальную метку, чтобы [игнорировать](#дедупликация-исключений) в дальнейшем.
