...
- **local_variables**: локальные переменные функции. Извлекаются автоматически при автоматическом логировании исключений, либо при ручном, если вы передадите в функцию ```log()``` экземпляр исключения. Также представлены в виде json с указанием типов данных.
