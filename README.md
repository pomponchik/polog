...
- **```set()```**: общие настройки логгера.
  ...
  Метод ```set()``` принимает следующие именованные параметры:
    ...
    **fields_intersection** (bool) - поведение при пересечении различных [дополнительных полей](#добавляем-собственные-поля) под одинаковым именем. Если ```fields_intersection``` установлено в значение ```True```, то внутри движка может происходить перезапись полей, извлеченных ранее. В противном случае - нет.
...
- **```add_engine_fields()```**: регистрация новых [полей лога](#добавляем-собственные-поля), которые будут извлекаться внутри движка.

- **```delete_engine_fields()```**: удаление внутридвижковых дополнительных [полей лога](#добавляем-собственные-поля).

- **```get_in_place_fields()```**: получение словаря с [дополнительными полями лога](#добавляем-собственные-поля), которые извлекаются до вызова движка. Если не передавать в метод ничего, вернет все такие поля, а если передать одно или несколько имен (в качестве неименованных аргументов), то вернет только их.

- **```get_engine_fields()```**: получение словаря с [дополнительными полями лога](#добавляем-собственные-поля), которые извлекаются внутри движка. По аналогии с ```get_in_place_fields()```, если не передавать в метод ничего, вернет все такие поля, а если передать одно или несколько имен (в качестве неименованных аргументов), то вернет только их.

- **```get_all_fields()```**: получить словарь с [дополнительными полями лога](#добавляем-собственные-поля) всех видов (то есть как извлекаемых внутри движка, так и до). По аналогии с ```get_in_place_fields()``` и ```get_engine_fields()```, если не передавать в метод ничего, вернет все такие поля, а если передать одно или несколько имен (в качестве неименованных аргументов), то вернет только их. В случае, если два разных экстрактора для извлечения вне движка и внутри него зарегистрированы под одинаковыми именами, данный метод учитывает значение настройки
...
Кроме того, вы можете указывать дополнительные поля не для всей программы целиком, а для конкретного декоратора. На том же примере с обработчиком запросов Django это будет выглядеть так:
...
В качестве аргумента ```extra_fields``` можно передать либо словарь как в примере выше, либо список или кортеж таких словарей. В случае, если вы указываете для конкретного декоратора набор полей отдельно, поля, установленные глобально, использоваться не будут. Если вы хотите, чтобы локальный набор полей добавился к глобальному (а в случае совпадения имен - перекрыл), нужно использовать троеточие, вот так:
