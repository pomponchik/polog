...
Отдельные важные события в вашем коде вы можете регистрировать вручную при помощи функции ```log()```. ```log()``` отличается от ```message()``` главным образом тем, что последняя редактирует лог, записываемый через декоратор, а ```log()``` создает инициирует новую запись лога.
