...
- Есть **более 520 тестов**, которые обеспечивают покрытие более чем на 95%. И стремление эти цифры еще увеличить.
...
Ее можно использовать для обычного [ручного](#ручное-логирование) логирования, в качестве декоратора [для функций](#декорируем-функции) (в том числе корутинных) и [классов](#декорируем-классы), а также в качестве [контекстного менеджера](#контекстный-менеджер).
...
Если вам не хочется в лишний раз импортировать ```@unlog```, можно воспользоваться уже импортированным объектом [```log```](#log---одна-функция-чтобы-править-всеми), подставляя ```-``` перед ним:
