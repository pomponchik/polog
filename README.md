...
На любые функцию, метод или даже класс вы можете навесить декоратор ```@unlog```, чтобы быть уверенными, что тут не будут срабатывать декораторы логирования. Это удобно, когда вы хотите, к примеру, временно приостановить логирование какой-то функции, не снимая логирующего декоратора.
...
```@unlog``` сработает при любом расположении среди декораторов логирования:
...
На классы тоже можно навешивать ```@unlog```:
...
Имейте ввиду, что ```@unlog``` "узнает" функции по их id. Это значит, что, если вы задекорируете функцию каким-то сторонним декоратором после того, как она помечена в качестве нелогируемой, декораторы Polog будут относиться к ней как к незнакомой:
