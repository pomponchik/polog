...
class AbstractToken(metaclass=MetaToken):
    ...
    Токены не должны отличаться для вызывающего кода ничем, вызывающий код не должен как-то учитывать особенности реализации токена.
    ...
    # Символы '*' (звездочка) и '.' (точка), а также '[' и ']', зарезервированы для использования внутри движка регулярных выражений.
